...
type
  Tf_hc_ccp3b = class(TForm)
    ...
    cxGOutDBTv_ccp3b_hid: TcxGridDBColumn;
    cxGOutDBTv_ccp3b_outid: TcxGridDBColumn;
    ...
    cxGDDBTv_ccp3b_did: TcxGridDBColumn;
    cxGDDBTv_ccp3b_hid: TcxGridDBColumn;
    ...
    uq_out: TUniQuery;
    ds_out: TDataSource;
    cxGInfoDBBTv_temp4: TcxGridDBBandedColumn;
    cxGDDBTv_temp4: TcxGridDBColumn;
    uq_list: TUniQuery;
...
procedure Tf_hc_ccp3b.FormCreate(Sender: TObject);
begin
...
procedure Tf_hc_ccp3b.cxBtn_AddDClick(Sender: TObject);
begin
  // 상세 목록 신규추가
  srch_D(uq_d, hid);
  // 레코드 추가
  uq_d.Append;
  uq_d.FieldByName('ccp3b_hid').AsInteger  := hid;
...
procedure Tf_hc_ccp3b.cxBtn_AddOutClick(Sender: TObject);
begin
  // 이탈 목록 신규추가
  srch_D(uq_out, hid);
  // 레코드 추가
  uq_out.Append;
  uq_out.FieldByName('ccp3b_hid').AsInteger := hid;
...
procedure Tf_hc_ccp3b.cxBtn_DelDClick(Sender: TObject);
begin
  // 상세 목록 삭제
  if MessageDlg(MessageToDeleteOne, mtWarning, [mbYes, mbNo], 0) = mrYes then
  begin
    if StrToIntDef(VarToStr(ddc.Values[ddc.FocusedRecordIndex, cxGDDBTv_ccp3b_did.Index]), 0) = 0 then
    begin
    ...
    begin
      ...
      begin
        ...
        ParamByName('ccp3b_did').AsInteger := ddc.Values[ddc.FocusedRecordIndex, cxGDDBTv_ccp3b_did.Index];
        ExecSQL;
...
procedure Tf_hc_ccp3b.cxBtn_DelOutClick(Sender: TObject);
begin
  // 이탈 목록 삭제
  if MessageDlg(MessageToDeleteOne, mtWarning, [mbYes, mbNo], 0) = mrYes then
  begin
    if StrToIntDef(VarToStr(odc.Values[odc.FocusedRecordIndex, cxGOutDBTv_ccp3b_outid.Index]), 0) = 0 then
    begin
    ...
    begin
      ...
      begin
        ...
        ParamByName('ccp3b_outid').AsInteger := odc.Values[odc.FocusedRecordIndex, cxGOutDBTv_ccp3b_outid.Index];
        ExecSQL;
...
procedure Tf_hc_ccp3b.Srch_D(uq: TUniQuery; val: Integer);
begin
  With DataModuleForm.FDQuerySetting(uq) do
  begin
    SQL.Add(SQLInsert.Text);
    ParamByName('ccp3b_hid').AsInteger := val;
    Open;
  end;
end;

procedure Tf_hc_ccp3b.Clear_All;
begin
  // CCP-3B Head
  Srch_D(uq_info, hid);
  // CCP-3B Detail
...
procedure Tf_hc_ccp3b.ClearProc;
begin
  // 신규
  vNew := True;
  // CCP-3B Head
  ...
  begin
    ...
    ParamByName('ccp3b_hid').AsInteger  := hid;
    Open;
  end;
//  // 쿼리 오픈
//  hid := 0;
...
//  uq_info.FieldByName('temp4').AsString       := '';
...
procedure Tf_hc_ccp3b.SaveProc;
var
  i, idx : Integer;
begin
  // 점검정보 저장
  ...
  begin
    SQL.Add(SQLInsert.Text);
    ParamByName('ccp3b_hid').AsInteger := StrToIntDef(VarToStr(idc.Values[idc.FocusedRecordIndex, cxGInfoDBBTv_ccp3b_hid.Index]), 0);
    ...
    ParamByName('temp4').AsString      := VarToStrDef(idc.Values[idc.FocusedRecordIndex, cxGInfoDBBTv_temp4.Index], '');
  ...
  begin
    ...
    begin
      ...
      ParamByName('ccp3b_did').AsInteger := StrToIntDef(VarToStr(ddc.Values[i, cxGDDBTv_ccp3b_did.Index]), 0);
      ParamByName('ccp3b_hid').AsInteger := idx;
      ...
      ParamByName('temp4').AsString      := VarToStrDef(ddc.Values[i, cxGDDBTv_temp4.Index], '');
  ...
  begin
    ...
    begin
      ...
      ParamByName('ccp3b_outid').AsInteger := StrToIntDef(VarToStr(odc.Values[i, cxGOutDBTv_ccp3b_outid.Index]), 0);
      ParamByName('ccp3b_hid').AsInteger   := idx;
...
procedure Tf_hc_ccp3b.DeleteProc;
begin
  // 점검 목록 삭제
  if MessageDlg(MessageToDeleteOne, mtWarning, [mbYes, mbNo], 0) = mrYes then
  begin
...
procedure Tf_hc_ccp3b.GridListSetting;
begin
  //
end;

procedure Tf_hc_ccp3b.DataBindingToGrid;
begin
  // 목록 선택
  vNew := False;
  // CCP-3B List
  ...
  begin
    ...
    hid := uq_list.FieldByName('ccp3b_hid').AsInteger;
  end;
  Clear_All;
end;

initialization
  RegisterClass(Tf_hc_ccp3b);

finalization
  UnRegisterClass(Tf_hc_ccp3b);
