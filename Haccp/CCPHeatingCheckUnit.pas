...
uses
  ...
  cxSplitter, MemDS, DBAccess, Uni, UniProvider, SQLServerUniProvider;

type
  TCCPHeatingCheckForm = class(TBaseChildForm)
    DS_Data: TDataSource;
...
var
  CCPHeatingCheckForm: TCCPHeatingCheckForm;
...
procedure TCCPHeatingCheckForm.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TCCPHeatingCheckForm.cxBtn_NewClick(Sender: TObject);
begin
  inherited;
...
//auto seq
procedure TCCPHeatingCheckForm.cxGridDBTv_data_noGetDisplayText(
...
procedure TCCPHeatingCheckForm.cxLkCBBox_FacilityPropertiesEditValueChanged(
  Sender: TObject);
begin
  inherited;
  ...
  with DataModuleForm.FDQuerySetting(FDQuery_comm) do
  begin
    sql.Add(' SELECT TOP 1 Temp,checkdate  ');
...
//해당월의 데이터 목록
procedure TCCPHeatingCheckForm.dxDateTimeWheelPickerPropertiesEditValueChanged(
  Sender: TObject);
begin
  inherited;
  ...
  begin
    sql.Add(' exec usp_CCPCheckHeating @option = :option, @TempDate=:TempDate ');
...
//일자 변경 시
procedure TCCPHeatingCheckForm.cxBtn_searchClick(Sender: TObject);
begin
  inherited;
  ...
  begin
    sql.Add(' exec usp_CCPCheckHeating @option = :option, @TempDate=:TempDate, @FridgeCode = :FridgeCode ');
...
//시간 및 온도 세팅
procedure TCCPHeatingCheckForm.SearchDateTempList;
  var ChkCnt : integer;
begin
  with DataModuleForm.FDQuerySetting(FDQuery_comm) do
  begin
    sql.Add(' EXEC usp_CCPCheckHeating @OPTION = :option           ');
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          sql.Add(' ORDER BY CheckDate DESC         ');
          ParamByName('CheckDate').AsDateTime := StrToDateTime(cxGridTv_temp.DataController.Values[loopcnt, cxGridTv_temp_CheckDate.Index]);
          ParamByName('FridgeNo').AsInteger  := cxLkCBBox_Facility.EditValue;
          open;
...
//한계기준 이탈 사유 및 조치내역 조회
procedure TCCPHeatingCheckForm.GetReasonActionContentsList;
...
//입력
procedure TCCPHeatingCheckForm.cxBtnProdOKClick(Sender: TObject);
begin
  inherited;
...
procedure TCCPHeatingCheckForm.cxBtnQAOKClick(Sender: TObject);
begin
  inherited;
  cxEdit_Writer.Text := LoginUserName;
...
procedure TCCPHeatingCheckForm.cxBtn_SaveClick(Sender: TObject);
begin
  inherited;
  ...
  for loopcnt := 0 to cxGridTv_temp.DataController.RecordCount -1 do
  begin
    with DataModuleForm.FDSpSetting(nil, 'usp_CCPCheckHeating', 'I') do
    begin
      if (VarToStr(cxGridTv_temp.DataController.Values[loopcnt, cxGridTv_temp_basic.Index]    ) <> '') or
...
//확인
procedure TCCPHeatingCheckForm.cxBtn_ConfirmerClick(Sender: TObject);
begin
  inherited;
...
procedure TCCPHeatingCheckForm.cxGridDBTv_dataCellDblClick(
...
//한계 기준 온도 벗어나면 표시 하기
procedure TCCPHeatingCheckForm.cxGridTv_tempCustomDrawCell(
...
procedure TCCPHeatingCheckForm.cxGridTv_tempEditing(Sender: TcxCustomGridTableView;
  AItem: TcxCustomGridTableItem; var AAllow: Boolean);
// 에디팅 허용
var
  DatePlusTime: string;
...
procedure TCCPHeatingCheckForm.cxGridTv_temp_EditValueChanged(
  Sender: TObject);
begin
  inherited;
...
//버튼들 활성화 여부
procedure TCCPHeatingCheckForm.SetApprobalButtons;
begin
  //결재버튼 활성화
