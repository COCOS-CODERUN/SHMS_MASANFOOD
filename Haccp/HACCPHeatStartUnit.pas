...
begin
  ...
  begin
    if y.Values[y.FocusedRecordIndex, cxGridTv_List_State.Index] = null then
    begin
      Exit;
    end;
...
//    Parambyname('h_id').asString        := CCPHeatID;
    ...
    ParamByName('GroupIndex').AsInteger := GroupIndex; //당일 지우지않고 그다음날 작업을 지울수도 있기때문에 now는 안됨
    open;
    ...
    begin
      ...
      begin
        SQL.Add(' and  Division = ''살균기1''      ');
      end
      else if GroupIndex = 2 then
      begin
        SQL.Add(' and Division = ''살균기2''      ');
      end
      else if GroupIndex = 3 then
      begin
        SQL.Add(' and Division = ''살균기3''      ');
      end;
...
//  US_List.Refresh;
//  y.BeginUpdate;
//  for Index := 0 to y.RecordCount -1 do
//  begin
//    US_List.First;
//    if (VarToStr(y.Values[Index, cxGridTv_List_WorkTime.Index]) <> '') and (VarToStr(y.Values[Index, cxGridTv_List_ItemName.Index]) <> '') then
//    begin
//      y.Values[Index, cxGridTv_List_HeatTime.Index] := US_List.FieldByName('NowTime').AsString;
//      US_List.Next;
//    end;
//  end;
//  y.EndUpdate;
