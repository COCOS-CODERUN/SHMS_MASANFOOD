...
class MssqlClient {
  ...
  Future<bool> connect({int loginTimeoutSeconds = 15}) async {
    ...
    try {
      ...
      // Query timeout
      try {
        final rc = _db!.dbsettime(loginTimeoutSeconds);
        MssqlLogger.i(
          'connect | op=dbsettime | seconds=$loginTimeoutSeconds | rc=$rc',
        );
      } catch (e) {
        MssqlLogger.w('connect | op=dbsettime | error=$e');
      }
      ...
      try {
        ...
        MssqlLogger.i('connect | op=dbsetlcharset');
        final sc = _db!.dbsetlcharset(login, c);
        MssqlLogger.i('connect | op=dbsetlcharset | charset=utf8 | rc=$sc');

        // 로그인 설정 검증
        if (su != SUCCEED || sp != SUCCEED || sc != SUCCEED) {
          MssqlLogger.e(
            'connect | op=login-options | su=$su | sp=$sp | sc=$sc | error=fail',
          );
          return false;
        }
  ...
  Future<String> execute(String sql, {bool useLastResultSet = false}) async {
    _ensureConnected();
    final db = _db!;
    final dbproc = _dbproc!;
    ...
    if (plan.needsSet) {
      ...
      try {
        ...
        return _collectResults(db, dbproc, useLastResultSet: useLastResultSet);
      } finally {
        malloc.free(cmd);
      }
    } else {
  ...
  Future<String> executeParams(
    ...
    // Plain query
    if (norm.isEmpty) {
      return execute(sql, useLastResultSet: useLastResultSet);
    }
    ...
    // Empty strings
    final emptyStringNames = norm.entries
        .where(
          (entry) => entry.value is String && (entry.value as String).isEmpty,
        )
        .map((entry) => entry.key)
        .toList(growable: false);
    final sqlForRpc = emptyStringNames.isEmpty
        ? sql
        : '${emptyStringNames.map((name) => "SET $name = N'';").join('\n')}\n$sql';

    // Prepare RPC call: sp_executesql(@stmt, @params, <params...>) with dynamic string encoding
    final rpcName = 'sp_executesql'.toNativeUtf8();
    final _StringDbBuf stmtBuf = _encodeUtf8String(sqlForRpc);
    final _StringDbBuf paramsBuf = _encodeUtf8String(declStr);
    ...
    try {
      ...
      for (final e in norm.entries) {
        ...
        final rpcVal = _encodeForRpc(
          value is String && value.isEmpty ? ' ' : value,
        );
  ...
  /// - Captures the first result set with columns by default. Callers can opt in
  ///   to the last result set for batches that append an output SELECT.
  /// - Row counts from all sets are aggregated via dbcount().
  ...
  String _collectResults(
    ...
    while (true) {
      ...
      final captureThisSet = ncols > 0 && (useLastResultSet || !capturedSet);

      // Last result reset
      if (captureThisSet && useLastResultSet) {
        columns.clear();
        rows.clear();
      }
...
class _StringDbBuf {
  final int type; // SYBVARCHAR
  final _TempBuf buf;
  _StringDbBuf(this.type, this.buf);
}

// UTF-8 문자열
_StringDbBuf _encodeUtf8String(String s) {
  final bytes = utf8.encode(s);
  final p = malloc<Uint8>(bytes.length);
  p.asTypedList(bytes.length).setAll(0, bytes);
