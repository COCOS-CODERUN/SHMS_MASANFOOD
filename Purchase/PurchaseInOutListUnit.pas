...
uses
  ...
  cxMaskEdit, cxCalendar, cxStyles, cxCustomData, cxFilter, cxData, System.DateUtils,
  ...
  cxGridDBTableView, cxClasses, cxGridCustomView, cxGrid, Uni, frxClass,
  frxDBSet, frxExportPDF, frxExportBaseDialog, frxExportXLSX, MemDS, DBAccess;

type
  TPurchaseInOutListForm = class(TBaseChildForm)
...
var
  PurchaseInOutListForm: TPurchaseInOutListForm;
...
procedure TPurchaseInOutListForm.FormCreate(Sender: TObject);
begin
  inherited;
  ...
  begin
    ...
    SQL.Add(' AND CustomerDivision in(''BD002'', ''BD022'') ');
    Open;
  end;
...
procedure TPurchaseInOutListForm.cxRGroup_GroupPropertiesChange(
  Sender: TObject);
begin
  inherited;
...
//excel
procedure TPurchaseInOutListForm.cxBtn_ExcelClick(Sender: TObject);
begin
  inherited;
...
//search
procedure TPurchaseInOutListForm.cxBtn_SearchClick(Sender: TObject);
  var CusID : Integer;
      OptionStr : string;
...
begin
  ...
  with DataModuleForm.FDSpSetting(SearchUniSP, 'usp_PurchaseInOut', OptionStr) do
  begin
...
//print
procedure TPurchaseInOutListForm.cxBtn_PrintClick(Sender: TObject);
begin
  inherited;
...
procedure TPurchaseInOutListForm.FUDS_SGetValue(const VarName: string;
  var Value: Variant);
begin
  inherited;
